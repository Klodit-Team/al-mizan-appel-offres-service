...
describe('AppelOffresController', () => {
  ...
  describe('uploadCdc', () => {
    it('doit lever une erreur si le fichier est manquant', async () => {
      await expect(
        controller.uploadCdc('ao-id', { prixRetrait: 0, fichier: null }, null as any)
    ...
    it('doit appeler le service uploadCdc avec les bons paramètres', async () => {
      ...
      const result = await controller.uploadCdc('ao-id', { prixRetrait: 250, fichier: mockFile }, mockFile);
  ...
  describe('getCdcDownloadUrl', () => {
    it('doit appeler le service avec un mock operateurId pour l\'instant', async () => {
      mockAppelOffresService.getPresignedDownloadUrl.mockResolvedValueOnce({ downloadUrl: 'http://test' });
