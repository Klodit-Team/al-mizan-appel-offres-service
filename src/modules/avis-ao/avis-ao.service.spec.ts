...
describe('AvisAoService', () => {
    ...
    describe('create', () => {
        it('devrait créer un avis', async () => {
            ...
            expect(prisma.avisAo.create).toHaveBeenCalledWith({ data: dto });
    ...
    describe('findAll', () => {
        it('devrait retourner tous les avis', async () => {
            ...
            expect(prisma.avisAo.findMany).toHaveBeenCalled();
            expect(result).toEqual([mockAvis]);
        });
    });
    ...
    describe('findOne', () => {
        it('devrait retourner un avis', async () => {
            ...
            expect(prisma.avisAo.findUnique).toHaveBeenCalledWith({ where: { id: 'test-id' } });
            expect(result).toEqual(mockAvis);
        });

        it('devrait lancer NotFoundException si non trouvé', async () => {
            (prisma.avisAo.findUnique as jest.Mock).mockResolvedValueOnce(null);
    ...
    describe('update', () => {
        it('devrait mettre à jour un avis', async () => {
            const result = await service.update('test-id', { contenuBomop: 'Updated' });
            expect(prisma.avisAo.update).toHaveBeenCalledWith({
                where: { id: 'test-id' },
    ...
    describe('remove', () => {
        it('devrait supprimer un avis', async () => {
            ...
            expect(prisma.avisAo.delete).toHaveBeenCalledWith({ where: { id: 'test-id' } });
            expect(result).toEqual(mockAvis);
        });
    });
});
