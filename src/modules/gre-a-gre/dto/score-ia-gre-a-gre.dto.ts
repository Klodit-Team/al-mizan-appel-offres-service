...
export class ScoreIaGreAGreDto {
  ...
  @ApiProperty({
    description: 'Nom/version du modèle IA utilisé (ex: gpt-4o, mistral-7b)',
  })
  @IsString({
    message: 'Le nom du modèle IA doit être une chaîne de caractères',
  })
  modeleIa: string;

  @ApiProperty({
    description: 'Score de conformité de 0 à 100',
    minimum: 0,
    maximum: 100,
  })
  ...
  @Max(100, {
    message: 'Le score de conformité doit être inférieur ou égal à 100',
  })
  scoreConformite: number;

  @ApiProperty({ enum: RecommandationIa })
  @IsEnum(RecommandationIa, {
    message: 'La recommandation doit être APPROUVER ou REJETER',
  })
  recommandation: RecommandationIa;

  @ApiProperty({
    description: 'Justification textuelle produite par le modèle IA',
  })
  @IsString({ message: 'La justification doit être une chaîne de caractères' })
  justificationIa: string;

  @ApiProperty({
    description: 'Score de confiance du modèle (0 à 100)',
    minimum: 0,
    maximum: 100,
  })
  ...
  @Max(100, {
    message: 'Le score de confiance doit être inférieur ou égal à 100',
  })
