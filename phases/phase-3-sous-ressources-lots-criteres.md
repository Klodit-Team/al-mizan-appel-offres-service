...
- _Règle métier_ : Un AO ne peut recevoir des lots que s'il est au statut `BROUILLON`. Teste l'état du parent avant création !
- _Validation_ : Si l'AO n'existe pas, renvoyer `404 Not Found`.
...
- _Validation Métier stricte (À implémenter dans la Phase 2 lors de la publication)_ : La **somme des `poids`** de tous les critères d'évaluation rattachés à un AO/Lot **doit être égale à 100**. Sinon, bloquer la publication avec `400 Bad Request`.
